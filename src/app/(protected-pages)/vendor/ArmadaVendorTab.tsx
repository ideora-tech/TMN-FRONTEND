...
export default function ArmadaVendorTab() {
    const router = useRouter()
    const searchParams = useSearchParams()
    const initialIdVendor = searchParams.get('id_vendor') ?? ''

    const [list, setList]             = useState<ArmadaVendor[]>([])
    ...
    const columns: ColumnDef<ArmadaVendor>[] = [
        {
            ...
            cell: (props: CellContext<ArmadaVendor, unknown>) =>
                props.row.index + 1 + (currentPage - 1) * pageSize,
        },
        {
            header: 'Nopol', accessorKey: 'nopol', size: 160,
            cell: ({ row }) => <span className="font-mono font-semibold">{row.original.nopol}</span>,
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => row.original.merk ?? <span className="text-gray-400">—</span>,
        },
        {
            header: 'Jenis', accessorKey: 'jenis',
            cell: ({ row }) => row.original.jenis ?? <span className="text-gray-400">—</span>,
        },
        {
            header: 'Kapasitas', accessorKey: 'kapasitas', size: 130,
            cell: ({ row }) => row.original.kapasitas ?? <span className="text-gray-400">—</span>,
        },
        {
            header: 'Tahun', accessorKey: 'tahun', size: 100,
            cell: ({ row }) => row.original.tahun ?? <span className="text-gray-400">—</span>,
        },
        {
                ...
                <div className="flex items-center justify-end">
                    <Tooltip title="Detail">
                        <span
                            ...
                            onClick={() => router.push(ROUTES.ARMADA_VENDOR_DETAIL(row.original.id_armada_vendor))}
                        >
