...
export default function JenisPerawatanBaruPage() {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        try {
            await jenisPerawatanService.create({
                nama: form.nama,
                keterangan: form.keterangan.trim() ? form.keterangan.trim() : null,
            })
            toast.push(<Notification type="success" title="Jenis Perawatan berhasil ditambahkan" />)
            router.push(ROUTES.JENIS_PERAWATAN)
        } catch (err) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-bold">Tambah Jenis Perawatan</h3>
                    <p className="text-gray-500 text-sm mt-0.5">Daftarkan jenis perawatan baru</p>
                </div>
            </div>
            <Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
                    <FormItem label="Nama Jenis Perawatan" asterisk invalid={!!errors.nama} errorMessage={errors.nama}>
                        <Input placeholder="Contoh: Ganti Oli, Service Berkala" value={form.nama} invalid={!!errors.nama}
