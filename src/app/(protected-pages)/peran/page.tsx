...
export default function PeranPage() {
    ...
    const [list, setList]             = useState<Peran[]>([])
    ...
    const [deleteTarget, setDeleteTarget] = useState<Peran | null>(null)
    ...
    const handleDelete = async () => {
        ...
        setSubmitting(true)
        try {
            await peranService.delete(deleteTarget.id_peran)
            toast.push(<Notification type="success" title="Peran berhasil dihapus" />)
    ...
    const columns: ColumnDef<Peran>[] = [
        { header: 'No', id: 'no', size: 60,
            cell: ({ row }: CellContext<Peran, unknown>) => (currentPage - 1) * pageSize + row.index + 1 },
        { header: 'Nama Peran', accessorKey: 'nama_peran', size: 220,
            cell: ({ row }: CellContext<Peran, unknown>) => {
                const initials = row.original.nama_peran.split(' ').slice(0, 2).map((w: string) => w[0]).join('').toUpperCase()
                return (
        ...
        { header: 'Kode', accessorKey: 'kode_peran', size: 130,
            cell: ({ row }: CellContext<Peran, unknown>) => (
                <span className="font-mono text-sm text-gray-600 dark:text-gray-400">{row.original.kode_peran}</span>
            ),
        },
        { header: 'Tipe', accessorKey: 'is_platform', size: 120,
            cell: ({ row }: CellContext<Peran, unknown>) => (
                <Tag className={row.original.is_platform
                    ...
                    {row.original.is_platform ? 'Platform' : 'Perusahaan'}
                </Tag>
            ),
        },
                ...
                <div className="flex items-center justify-end gap-2">
                    <Tooltip title="Kelola Akses">
                        <span className="cursor-pointer inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/20 dark:text-blue-300 dark:hover:bg-blue-500/30 transition-colors"
                            onClick={() => router.push(ROUTES.PERAN_DETAIL(row.original.id_peran))}>
        ...
        <div className="flex flex-col gap-4">
            ...
            <Card bodyClass="p-0">
                <div className="flex flex-wrap items-center gap-3 px-4 py-3">
                    <Input className="flex-1 min-w-60" placeholder="Cari kode atau nama peran... (tekan Enter)"
                        suffix={searchInput
            ...
            <ConfirmDialog isOpen={!!deleteTarget} type="danger" title="Hapus Peran?"
                ...
                <p className="text-sm">Peran <span className="font-semibold">&ldquo;{deleteTarget?.nama_peran}&rdquo;</span> akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
            </ConfirmDialog>
        </div>
    )
}