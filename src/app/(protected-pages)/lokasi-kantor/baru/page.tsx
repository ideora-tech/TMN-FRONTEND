...
export default function LokasiKantorBaruPage() {
    const router = useRouter()
    const [form, setForm] = useState({
        kode_lokasi: '', nama_lokasi: '', alamat: '', kota: '',
        latitude: '', longitude: '', radius: '100', aktif: true,
    })
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        try {
            await lokasiKantorService.create({
            ...
            toast.push(<Notification type="success" title="Lokasi kantor berhasil ditambahkan" />)
            router.push(ROUTES.LOKASI_KANTOR)
        } catch (err) {
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-bold">Tambah Lokasi Kantor</h3>
                    <p className="text-gray-500 text-sm mt-0.5">Daftarkan lokasi kantor baru</p>
                </div>
            </div>
            <Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
                    <FormItem label="Kode Lokasi" asterisk invalid={!!errors.kode_lokasi} errorMessage={errors.kode_lokasi}>
                        <Input placeholder="Kode unik" value={form.kode_lokasi} invalid={!!errors.kode_lokasi}
                            onChange={e => setForm(p => ({ ...p, kode_lokasi: e.target.value }))} />
                    </FormItem>
                    ...
                    <FormItem label="Radius Absensi (meter)">
                        <Input type="number" min={0} placeholder="100" value={form.radius}
                            onChange={e => setForm(p => ({ ...p, radius: e.target.value }))} />
                    </FormItem>
                    <FormItem label="Latitude">
                        <Input type="number" step="any" placeholder="-6.200000" value={form.latitude}
                            onChange={e => setForm(p => ({ ...p, latitude: e.target.value }))} />
                    </FormItem>
                    <FormItem label="Longitude">
                        <Input type="number" step="any" placeholder="106.800000" value={form.longitude}
