...
export default function DepartemenPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true)
        try {
            const res = await departemenService.list(currentPage)
            setList(res.data)
    ...
    const handleDelete = async () => {
        ...
        setSubmitting(true)
        try {
            await departemenService.delete(deleteTarget.id_departemen)
            toast.push(<Notification type="success" title="Departemen berhasil dihapus" />)
    ...
    const filteredList = list.filter(d => {
        const matchSearch = !search ||
            d.kode_departemen.toLowerCase().includes(search.toLowerCase()) ||
            d.nama_departemen.toLowerCase().includes(search.toLowerCase())
        const matchAktif = aktifFilter === '' || String(d.aktif ? 1 : 0) === aktifFilter
        return matchSearch && matchAktif
    })

    const columns: ColumnDef<Departemen>[] = [
        { header: 'No', id: 'no', size: 60,
            cell: ({ row }: CellContext<Departemen, unknown>) => (currentPage - 1) * pageSize + row.index + 1 },
        { header: 'Nama Departemen', accessorKey: 'nama_departemen', size: 240,
            cell: ({ row }: CellContext<Departemen, unknown>) => {
                const initials = row.original.nama_departemen.split(' ').slice(0, 2).map((w: string) => w[0]).join('').toUpperCase()
                return (
        ...
        { header: 'Kode', accessorKey: 'kode_departemen', size: 130,
            cell: ({ row }: CellContext<Departemen, unknown>) => (
                <span className="font-mono text-sm text-gray-600 dark:text-gray-400">{row.original.kode_departemen}</span>
            ),
        },
                ...
                <div className="flex items-center justify-end gap-2">
                    <Tooltip title="Detail">
                        <span className="cursor-pointer inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/20 dark:text-blue-300 dark:hover:bg-blue-500/30 transition-colors"
                            onClick={() => router.push(ROUTES.DEPARTEMEN_DETAIL(row.original.id_departemen))}>
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <ConfirmDialog isOpen={!!deleteTarget} type="danger" title="Hapus Departemen?"
                ...
                <p className="text-sm">Departemen <span className="font-semibold">&ldquo;{deleteTarget?.nama_departemen}&rdquo;</span> akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
            </ConfirmDialog>
        </div>
    )
}