...
export default function TipePembayaranPage() {
    const router = useRouter()
    const [list, setList]             = useState<TipePembayaran[]>([])
    ...
    const [deleteTarget, setDeleteTarget] = useState<TipePembayaran | null>(null)

    const fetchData = useCallback(async () => {
        setLoading(true)
        try {
            const res = await tipePembayaranService.list(currentPage, pageSize, search, aktifFilter)
            setList(res.data)
    ...
    const handleDelete = async () => {
        ...
        setSubmitting(true)
        try {
            await tipePembayaranService.delete(deleteTarget.id_tipe_pembayaran)
            toast.push(<Notification type="success" title="Tipe pembayaran berhasil dihapus" />)
    ...
    const columns: ColumnDef<TipePembayaran>[] = [
        { header: 'No', id: 'no', size: 60,
            cell: ({ row }: CellContext<TipePembayaran, unknown>) => (currentPage - 1) * pageSize + row.index + 1 },
        { header: 'Nama Tipe', accessorKey: 'nama_tipe', size: 260,
            cell: ({ row }: CellContext<TipePembayaran, unknown>) => {
        ...
        { header: 'Kode', accessorKey: 'kode_tipe', size: 160,
            cell: ({ row }: CellContext<TipePembayaran, unknown>) => (
                <span className="font-mono text-sm text-gray-600 dark:text-gray-400">{row.original.kode_tipe}</span>
            ),
        },
                ...
                <div className="flex items-center justify-end gap-2">
                    <Tooltip title="Detail">
                        <span className="cursor-pointer inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/20 dark:text-blue-300 dark:hover:bg-blue-500/30 transition-colors"
                            onClick={() => router.push(ROUTES.TIPE_PEMBAYARAN_DETAIL(row.original.id_tipe_pembayaran))}>
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h3 className="font-bold">Tipe Pembayaran</h3>
                    <p className="text-gray-500 text-sm mt-0.5">Data master tipe pembayaran invoice vendor</p>
                </div>
                <Button variant="solid" size="sm" icon={<HiPlusCircle />}
                    onClick={() => router.push(ROUTES.TIPE_PEMBAYARAN_BARU)}>
                    Tambah Tipe
                </Button>
            </div>
            <Card bodyClass="p-0">
                <div className="flex flex-wrap items-center gap-3 px-4 py-3">
                    <Input className="flex-1 min-w-60" placeholder="Cari kode atau nama tipe... (tekan Enter)"
                        suffix={searchInput
            ...
            <ConfirmDialog isOpen={!!deleteTarget} type="danger" title="Hapus Tipe Pembayaran?"
                ...
                <p className="text-sm">Tipe pembayaran <span className="font-semibold">&ldquo;{deleteTarget?.nama_tipe}&rdquo;</span> akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
            </ConfirmDialog>
        </div>
    )
}
