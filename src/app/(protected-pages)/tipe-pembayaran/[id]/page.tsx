...
export default function TipePembayaranDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params)
    const router = useRouter()
    const [data, setData]     = useState<TipePembayaran | null>(null)
    const [loading, setLoading] = useState(true)
    const [editing, setEditing] = useState(false)
    const [form, setForm]       = useState<Partial<TipePembayaran>>({})
    const [errors, setErrors]   = useState<Partial<Record<keyof typeof form, string>>>({})
    const [saving, setSaving]   = useState(false)

    useEffect(() => {
        tipePembayaranService.get(id)
    ...
    const handleSave = async () => {
        ...
        setSaving(true)
        try {
            const updated = await tipePembayaranService.update(id, {
                kode_tipe: form.kode_tipe,
                nama_tipe: form.nama_tipe,
                aktif:     form.aktif,
            })
            setData(updated); setEditing(false); setErrors({})
            toast.push(<Notification type="success" title="Tipe pembayaran berhasil diperbarui" />)
        } catch (err) {
    ...
    if (!data) return <div className="p-6 text-red-500">Tipe pembayaran tidak ditemukan.</div>
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
                <button type="button" onClick={() => router.push(ROUTES.TIPE_PEMBAYARAN)}
                    className="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 transition-colors">
                    <HiArrowLeft className="text-xl" />
                </button>
                <div>
                    <h3 className="font-bold">{data.nama_tipe}</h3>
                    <p className="text-gray-500 text-sm mt-0.5">Kode: {data.kode_tipe}</p>
                </div>
            </div>
            <Card>
                {!editing ? (
                    <>
                        <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 mt-1">Kode: {data.kode_tipe}</p>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
                            {([
                                { label: 'Kode Tipe', value: data.kode_tipe },
                    ...
                    <>
                        <div className="flex items-center gap-4 mb-5">
                            ...
                            <div>
                                <p className="font-semibold text-base text-gray-800 dark:text-gray-100">Edit Tipe Pembayaran</p>
                                <p className="text-sm text-gray-500 mt-0.5">Perbarui informasi tipe pembayaran di bawah ini</p>
                            </div>
                        </div>
