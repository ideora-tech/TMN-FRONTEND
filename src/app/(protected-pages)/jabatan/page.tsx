...
export default function JabatanPage() {
    ...
    const handleDelete = async () => {
        ...
        setSubmitting(true)
        try {
            await jabatanService.delete(deleteTarget.id_jabatan)
            toast.push(<Notification type="success" title="Jabatan berhasil dihapus" />)
    ...
    const columns: ColumnDef<Jabatan>[] = [
        { header: 'No', id: 'no', size: 60,
            cell: ({ row }: CellContext<Jabatan, unknown>) => (currentPage - 1) * pageSize + row.index + 1 },
        { header: 'Nama Jabatan', accessorKey: 'nama_jabatan', size: 240,
            cell: ({ row }: CellContext<Jabatan, unknown>) => {
                const initials = row.original.nama_jabatan.split(' ').slice(0, 2).map((w: string) => w[0]).join('').toUpperCase()
                return (
        ...
        { header: 'Kode', accessorKey: 'kode_jabatan', size: 130,
            cell: ({ row }: CellContext<Jabatan, unknown>) => (
                <span className="font-mono text-sm text-gray-600 dark:text-gray-400">{row.original.kode_jabatan}</span>
            ),
        },
        { header: 'Level', accessorKey: 'level', size: 80 },
                ...
                <div className="flex items-center justify-end gap-2">
                    <Tooltip title="Detail">
                        <span className="cursor-pointer inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-500/20 dark:text-blue-300 dark:hover:bg-blue-500/30 transition-colors"
                            onClick={() => router.push(ROUTES.JABATAN_DETAIL(row.original.id_jabatan))}>
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <ConfirmDialog isOpen={!!deleteTarget} type="danger" title="Hapus Jabatan?"
                ...
                <p className="text-sm">Jabatan <span className="font-semibold">&ldquo;{deleteTarget?.nama_jabatan}&rdquo;</span> akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.</p>
            </ConfirmDialog>
        </div>
    )
}