...
export default function PapanShift({
    ...
    const [hapusSupirTarget, setHapusSupirTarget] = useState<SupirProyek | null>(null)
    const [menghapusSupir, setMenghapusSupir]     = useState(false)
    ...
    const handleKonfirmasiHapusSupir = async () => {
        if (!hapusSupirTarget) return
        setMenghapusSupir(true)
        try {
            await onHapusSupir(hapusSupirTarget)
            setHapusSupirTarget(null)
            refetch()
        } catch (err) {
            toast.push(<Notification type="danger" title={parseApiError(err)} />)
        } finally {
            setMenghapusSupir(false)
        }
    }
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            ...
            <ConfirmDialog isOpen={!!hapusSupirTarget} type="danger" title="Keluarkan Supir dari Proyek"
                confirmText="Ya, Keluarkan" cancelText="Batal"
                onClose={() => setHapusSupirTarget(null)}
                onCancel={() => setHapusSupirTarget(null)}
                onConfirm={handleKonfirmasiHapusSupir}
                confirmButtonProps={{ loading: menghapusSupir }}>
                <p>
                    Supir <strong>{hapusSupirTarget?.nama}</strong> akan dikeluarkan dari proyek ini — shift mulai
                    hari ini ikut terhapus, riwayat shift yang sudah lewat tetap tersimpan.
                </p>
            </ConfirmDialog>
        </div>
    )
